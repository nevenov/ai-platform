...
/**
 * Get or auto-start MCP client
 * Auto-starts from environment variables if available
 */
export async function getMCPClient(): Promise<MCPClient> {
  if (!mcpClientInstance) {
    // Try auto-start from environment variables
    ...
    if (host && port && username && database) {
      return startMCPClient({ host, port, username, password, database });
    }

    throw new Error(
      "MCP client not started. Please configure MySQL via /modules/mysql or set MYSQL_* environment variables."
    );
  }
